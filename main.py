...
def random_basis(n, m):
    ...
    t = []
    for i in range(m):
        t.append(basis[0][i] + basis[2][i] + 3)
...
def make_clauses_and_weights(basis, n):
    ...
    """
    k = len(basis)
    count = n+2     #this is used to introduce new variables in the clauses for y1 <-> x1 & x2. This represents these y variables for each such pair.  
    clauses = []
    ...
    for i in range(1, n+1):
        for j in range(i+1, n+2):
            ...
            wk = (wi+wj+2*inner_product(basis, i, j))
...
oracle_time = run_maxsat(len(basis)-1, basis)   #here n = 4, as the last vector is the target vector